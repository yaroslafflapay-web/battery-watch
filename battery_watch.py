...
def extract_soc(data):
    ...
    # Не нашли — печатаем все доступные параметры в лог запуска Action,
    # чтобы можно было вручную найти нужный id и прописать его в секреты.
    print(f"Параметр с id='{SOC_PARAM_ID}' не найден. Доступные параметры:")
    print(json.dumps(pars, ensure_ascii=False, indent=2))
    sys.exit(1)
